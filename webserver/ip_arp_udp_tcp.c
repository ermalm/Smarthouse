...
const char ippingreqhdr[] PROGMEM ={0x45,0,0,0x54,0,0,0x40,0,0x02,IP_PROTO_ICMP_V}; // 0x54 is the total len on ip, 0x02 is ttl (time to live)
...
//---------------- icmp client 
//all the above is for a server. the code below is to initiate a ping
//
...
void client_icmp_request(uint8_t *buf,uint8_t *destip,uint8_t matchpat)
{
        uint8_t i=0;
        uint16_t ck;
        //
        ...
        buf[ETH_TYPE_L_P] = ETHTYPE_IP_L_V;
        fill_buf_p(&buf[IP_P],10,ippingreqhdr);
        i=0;
        while(i<4){
